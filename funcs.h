...
struct scoreData{
    char handle[20];
    int score;
	//compare score and sort the player in descending order
    bool operator < (const scoreData &p) const {
        if(score == p.score) return strcmp(handle, p.handle) < 0;
        return score > p.score;
    }
};
std::set< scoreData > LeaderboardData;//declaring leaderboardData  as set
...
void loadScoreFromFile() {
	...
	FILE *readF = fopen("leaderboard.txt", "r");
...
void updateScores() {
	loadScoreFromFile();
	FILE *writeF = fopen("leaderboard.txt", "w");
...
bool init()
{
	...
	else
	{
		...
		//Create window
		gWindow = SDL_CreateWindow( "Treasure Hunt- DU", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
		if( gWindow == NULL )
		{
