	...
	else
	{
		...
		{	
			...
			{	
				...
				while( SDL_PollEvent( &e ) != 0 )//Poll for currently pending events
				{
					//User requests quit
					if( e.type == SDL_QUIT )
					{
					...
					if(where == MENU)
					{
						if(recall == 1)
						{
                            ...
                            Sprite.Life = 100; // Providing full life when restarting the game 

							renderText = false;
							inputText = " ";
							Sprite.init(); //Initializing the running character again
				...
				{	
					...
					if(PauseOn)
					{
						...
						SDL_Rect* currentClip = &Spriteclips[1];
						Sprite.render(currentClip);
						SDL_RenderPresent(gRenderer);
					}
					...
					else
					{
						...
						if(Sprite.fireball_throwed)
						{
							for(int i=0;i<2;i++)
							{
								if(!fb[i].flag_of_fireball)
								{
									...
									fb[i].mPosX=Sprite.mPosX+Sprite.Sprite_WIDTH;
									fb[i].mPosY=Sprite.mPosY+Sprite.Sprite_HEIGHT/2-5;
									break;
								}
							}
							Sprite.fireball_throwed=0;
						}
						...
						for(int i=0;i<10;i++)
						{
							if(barrier[i].flag_of_obstacle)
							{
								...
								//collision detection 
								if(checkCollision(Sprite.Sprite_Rect,barrier[i].Obstacle_rect) && !barrier[i].hitten)
								{
									barrier[i].hitten = 1;
									
									Sprite.Life-=25; //Reduces life in each event of collision

									health_path[7]++;
									gHealthTexture.loadFromFile(health_path);

									Sprite.hitten=77;
									cout << "Life :" << Sprite.Life << endl; //In each loss of life, the remaining lives will be showed in the terminal
								}
							}
						...
						for(int i=0;i<10;i++)
						{
							if(plane[i].flag_of_obstacle)
							{
								...
								if(checkCollision(Sprite.Sprite_Rect,plane[i].Moving_Obstacle_rect) && !plane[i].hitten)
								{
									plane[i].hitten = 1;
				...
				{	
					...
					else
					{
						gHealthTexture.loadFromFile(health_path);
						Sprite.hitten--;
						Sprite.hitten = max(0, Sprite.hitten);
						for(int i=0;i<10;i++)
						{
							if(!barrier[i].flag_of_obstacle)
							{
